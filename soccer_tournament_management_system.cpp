...
void display_report(const team teams[], int team_count, int players_per_team, int match_count)
{
    ...
    for (int team_index = 0; team_index < team_count; team_index++)
    {
        ...
        for (int players_index = 0; players_index < players_per_team; players_index++)
        {
            cout << teams[team_index].players[players_index].player_id
                << " "
                << teams[team_index].players[players_index].player_name
                << " - Goals: "
                << teams[team_index].players[players_index].goals_scored
                << endl;

            total_goals += teams[team_index].players[players_index].goals_scored;
        }
    ...
    cout << "\nTournamnet Winner: " << teams[winner_index].team_name << endl;
